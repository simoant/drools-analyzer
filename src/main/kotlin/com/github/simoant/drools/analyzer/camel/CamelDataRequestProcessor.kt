...
data class CamelDataRequestProcessor(val producerTemplate: ProducerTemplate) : IDataRequestProcessor {
    override fun executeAsync(request: DataRequest, trackId: String): CompletableFuture<Any?> {
        val futureRes = if (request.uri.startsWith("seda:"))
            producerTemplate.asyncSend(request.uri) { exchange ->
                exchange.pattern = ExchangePattern.InOnly
                exchange.`in`.body = request.requestData
                exchange.`in`.headers = mapOf(X_UUID_NAME to trackId)
            }
        else
            producerTemplate.asyncRequestBodyAndHeader(request.uri, request.requestData,
                X_UUID_NAME, trackId)
